HLPfit <- function(x, y,
                   id,
                   control = list(start = c(a = 1, b = 35), maxAttempts = 100),
                   groupsNull = NULL,
                   groupsAlt,
...
repeatSingleFit <- function(x, y,
  ...
  while (doFit){
    ...
    m <- fitExponential(x = x, y = y, start = startTmp)
    i <- i + 1
...
fitExponential <- function(x, y, start = c(a = 1, b = 20)){
  try(nls(formula = y ~ (1 - a * (1/2) ^ (x / b)),
          start = start,
          data = list(x=x, y=y)), 
          # na.action = na.omit,
          # algorithm = "port",
