...
gene.data <- read.delim("./res/293t/gene_data_p.txt")
gene.noms <- read.csv("./res/293t/Norm/data_noms.csv")
...
for (i in names(gene.data.list)) {
  ...
  ggsave(paste0("./res/293t/BoxPlot/FIG2C/", i, ".pdf"), width = 6, height = 2.75)
}
...
ggsave("./res/293t/BoxPlot/go.pdf", group.go.plot, width = 6, height = 10)
