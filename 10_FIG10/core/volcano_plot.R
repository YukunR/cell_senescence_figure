...
create_volcano_plot <- function(gene_data,
                                ...
                                show_stats = TRUE,
                                xlim_symmetric = TRUE,
                                regulation_column = NULL) {
  # Step 1: Classify regulation groups
  if (!is.null(regulation_column) && regulation_column %in% colnames(gene_data)) {
    # Use pre-computed regulation column (e.g. from annotate_delta_fc), which may
    # incorporate filters beyond simple FC/p thresholds (e.g. positive means filter).
    classified_data <- gene_data
    classified_data$log2_fc     <- classified_data[[fc_column]]
    classified_data$neg_log10_p <- -log10(classified_data[[p_column]])
    classified_data$regulation_group <- dplyr::case_when(
      classified_data[[regulation_column]] == "increased_in_senescent" ~ "up",
      classified_data[[regulation_column]] == "decreased_in_senescent" ~ "down",
      TRUE ~ "no_change"
    )
    summary_tbl <- classified_data %>%
      dplyr::count(regulation_group) %>%
      dplyr::mutate(percentage = round(n / sum(n) * 100, 1))
    attr(classified_data, "regulation_summary") <- summary_tbl
    attr(classified_data, "thresholds") <- list(
      fc_threshold      = fc_threshold,
      p_threshold       = p_threshold,
